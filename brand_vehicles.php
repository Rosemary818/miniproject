...
if (isset($_POST['toggle_wishlist'])) {
    ...
    if ($check_result->num_rows > 0) {
        ...
        $wish_sql = "DELETE FROM tbl_wishlist WHERE user_id = ? AND vehicle_id = ?";
    } else {
        // Add to tbl_wishlist
        $wish_sql = "INSERT INTO tbl_wishlist (user_id, vehicle_id) VALUES (?, ?)";
    }
    
    $wish_stmt = $conn->prepare($wish_sql);
    $wish_stmt->bind_param("ii", $user_id, $vehicle_id);
    $wish_stmt->execute();
    
    // Redirect to refresh the page and avoid form resubmission
    header("Location: brand_vehicles.php?brand=" . urlencode($brand));
    exit();
}
?>
...
<head>
    ...
    <title><?php echo htmlspecialchars($brand); ?> Vehicles - WheeleDeal</title>
    ...
    <div class="container">
        <div class="content-wrapper">
            ...
            <div class="main-content">
                <h2 class="section-title"><?php echo htmlspecialchars($brand); ?> Vehicles</h2>
                <div class="car-grid">
                    <?php
                    if ($result->num_rows > 0) {
                        ...
                        echo '<p class="no-cars">No ' . htmlspecialchars($brand) . ' vehicles available right now.</p>';
                    }
                    ?>
                </div>
            </div>
